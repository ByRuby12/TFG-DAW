  ...
  <div>
    <div>
      <PanelMenu
        ...
        :aviso-perfil="!perfilCompleto"
        :has-equipo="hasEquipo"
        :is-captain="isCaptain"
        :username="username"
        :is-admin="userRole==='administrador'"
        :is-arbitro="userRole==='arbitro'"
        :user-avatar="avatarUrl"
        @cerrar-sesion="handleLogout"
        @ver-perfil="estadoActual = 'perfil'"
        @crear-equipo="abrirCrearEquipo"
        @ver-mi-equipo="verMiEquipo"
        @unirse-equipo="estadoActual = 'inscribirse'"
        @ver-partidos="onVerPartidos"
        @panel-arbitro="estadoActual='panelArbitro'"
        @ver-inicio="estadoActual = 'home'"
        @ver-ajustes="estadoActual = 'ajustes'"
        @ver-estadisticas="estadoActual = 'estadisticas'"
        @ver-admin="estadoActual = 'admin'"
        @avatar-changed="onAvatarChanged"
        @ver-reportes="onVerReportes"
        @ver-reportes-admin="estadoActual = 'reportesAdmin'"
        @ver-estadios="estadoActual = 'estadioMapa'"
      >
      <Home
      v-if="estadoActual === 'home'"
      />
      <PanelPerfil
      v-if="estadoActual === 'perfil'"
      @volver-menu="estadoActual = 'home'"
      @perfil-guardado="onPerfilGuardado"
      />
      <PanelCrearEquipo
      v-if="estadoActual === 'crearEquipo'"
      @equipo-creado="onEquipoCreado"
      @volver-menu="estadoActual = 'home'"
      />
      <PanelVerEquipo
      v-if="estadoActual === 'verEquipo'"
      :equipo-id="miEquipoId"
      @volver-menu="estadoActual = 'home'"
      @salio-equipo="onEquipoSalio"
      />
      <PanelInscribirse
      v-if="estadoActual === 'inscribirse'"
      @equipo-unido="onEquipoUnido"
      @volver-menu="estadoActual = 'home'"
      />
      <PanelPartidos
      v-if="estadoActual === 'partidos'"
      :equipo-id="miEquipoId"
      @volver-menu="estadoActual = 'home'"
      />
      <PanelArbitro
      v-if="estadoActual ==='panelArbitro'"
      @cancelar="estadoActual='home'"
      />
      <PanelAdmin
      v-if="estadoActual === 'admin'"
      />
      <PanelEstadisticas
      v-if="estadoActual === 'estadisticas'"
      />
      <PanelAjustes
      v-if="estadoActual === 'ajustes'"
      />
      <PanelReportesUsuario
      v-if="estadoActual === 'reportes'"
      />
      <PanelReportesAdmin
      v-if="estadoActual === 'reportesAdmin'"
      />
      <PanelEstadios
      v-if="estadoActual === 'estadioMapa'"
      />
      </PanelMenu>
      <PanelRegister
        v-if="estadoActual === 'register'"
          @cambiar-panel="estadoActual = 'login'"
      />
      <PanelLogin
        v-if="estadoActual === 'login'"
        @cambiar-panel="estadoActual = 'register'"
        @loguear="comprobarLogin"
        @mostrar-recuperacion="estadoActual = 'recuperar'"
      />
      <PanelConfirmacion
      v-if="estadoActual === 'confirmacion'"
      @volver-login="volverAlLogin"
      />
      <PanelRecuperar
      v-if="estadoActual === 'recuperar'"
      @codigo-enviado="mostrarCodigo"
      @volver-login = "estadoActual = 'login'"
      />
      <PanelCodigo
      v-if="estadoActual === 'codigo'"
      :correo="correo"
      @codigo-verificado="mostrarNueva"
      />
      <PanelNuevaContrasena
      v-if="estadoActual === 'nueva'"
      :correo="correo"
      @volver-login="estadoActual= 'login'"
      />
    </div>
...
<style scoped>
/* Tailwind se encarga de casi todo. Puedes añadir ajustes aquí si lo necesitas. */
</style>